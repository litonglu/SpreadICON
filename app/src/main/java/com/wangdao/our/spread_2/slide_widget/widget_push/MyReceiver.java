...
import com.wangdao.our.spread_2.slide_widget.widget_push.TestActivity;
...
public class MyReceiver extends BroadcastReceiver {
	...
	private void processCustomMessage(Context context, Bundle bundle) {
		if (MessageA.isForeground) {
			...
			Intent msgIntent = new Intent(MessageA.MESSAGE_RECEIVED_ACTION);
			msgIntent.putExtra(MessageA.KEY_MESSAGE, message);
			if (!com.wangdao.our.spread_2.slide_widget.widget_push.ExampleUtil.isEmpty(extras)) {
				try {
					JSONObject extraJson = new JSONObject(extras);
					if (null != extraJson && extraJson.length() > 0) {
						msgIntent.putExtra(MessageA.KEY_EXTRAS, extras);
					}
				} catch (JSONException e) {
